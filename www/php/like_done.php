...
if (!empty($_POST['like_id'])) {
    ...
    $req = $bdd->prepare('INSERT INTO like_event_picture(id_account, id_event_picture) VALUES(:account_id, :like_id)');
} else if (!empty($_POST['unlike_id'])) {
    $like_id = $_POST['unlike_id'];
    $req = $bdd->prepare('DELETE FROM like_event_picture WHERE id_account=:account_id AND id_event_picture=:like_id LIMIT 1');
} else {
